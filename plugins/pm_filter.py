...
from utils import shortlink, get_shortlink, get_size, is_subscribed, get_poster, search_gagala, temp, get_settings, save_group_settings
...
async def auto_filter(client, msg, spoll=False):
    ...
    if VERIFY: 
        if settings["button"]:	    
            ...
            for file in files: 
                btn.append([ 
                    InlineKeyboardButton(text=f"[{get_size(file.file_size)}] {file.file_name}", url=f'https://telegram.dog/{temp.U_NAME}?start=files_{msg.chat.id}_{file.file_id}') 
                ])	
        ...
        if settings["button"]: 
            ...
            for file in files: 
                btn.append([ 
                    InlineKeyboardButton(text=f"[{get_size(file.file_size)}] {file.file_name}", url=f'https://telegram.dog/{temp.U_NAME}?start=Urllink_{msg.chat.id}_{file.file_id}') 
                ])	
        else:
            btn = [] 
            for file in files: 
